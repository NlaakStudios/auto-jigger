	...
	*/
	var Maestro2 = {
		...
		start:function(applet,target,scripts) {
			...
			this.xhr(applet,target);
		},
		
		/**
		 * called internally to create to load a script and attach to DOM.
		 *
		 * @param {String} f	URL of the script to load
		 * @param {String} t	element to attach the script to [head or tail]
		 * @private
		 * @memberof Maestro2
		 */			
		getScripts:function(f,t){
			var element=null;
			do{
				...
				element.onload=function(){
					console.log(Maestro2.gfn(this.src)+' loaded.')
				};
		...
		loadAllScripts:function(files) {
			...
			if(tail&&tail.tagName=="DIV"){
				if(files&&files.constructor.name==="Array"){
					Maestro2.addEvent(document.getElementsByTagName("head")[0], 'onload',  this.getScripts(files,tail));
				}
			}
		},
		...
		xhr : function ($url, $onLoad, $onError, $method, $data, $trace) {
			...
			makeCorsRequest = function ($url, $onLoad, $data) {
				...
				xhr.onloadend = function(){
					...
					Maestro2['cls'](Maestro2['VIEWPORT'],"replace","opacity0","opacity100"); /* ON */
					Maestro2['cls']("splash","replace","opacity100","opacity0"); /* OFF */
				}	
				...
				xhr.onerror = function() {
					Maestro2['cls'](Maestro2['VIEWPORT'],"replace","opacity100","opacity0"); /* OFF */
					Maestro2['cls']("splash","replace","opacity0","opacity100"); /* ON */
					Maestro2['gei']('modaltext').innerHTML="<h1>App <small>Error</small></h1><p>"+"Woops, there was an error making the request."+"</p>"
					$('#ErrorModal').modal({keyboard:false,backdrop:true,show:true})
				};
