	...
	(typeof window !== "undefined" ? window : this, function (window, noGlobal) {
		...
		maestro.log = function ($id, $type, $args) {
			...
			if ($args.constructor.name == 'ErrorEvent') {
				$m = maestro.prototype.dmp($args, 1);
			...
			if ($type == "INFO") {
				$nr.className += 'text-info';
				$nt0 = maestro.s[5][6][1] + 'info-sign">'
			} else if ($type == "ERROR") {
				$nr.className += 'text-danger';
				$nt0 = maestro.s[5][6][1] + 'remove-sign">'
			} else if ($type == "WARN") {
				$nr.className += 'text-warning';
				$nt0 = maestro.s[5][6][1] + 'exclamation-sign">'
			} else {
				$type = "DEBUG";
				$nr.className += 'text-default';
				$nt0 = maestro.s[5][6][1] + 'wrench">'
			}
		...
		maestro.pf = maestro.prototype = {
			...
			dmq : function ($p) {
				...
				$p.a.className = 'closed opacity20';
		...
		maestro.fn = maestro.prototype = {
			 ...
			 */
			gei : function ($p) {
				return maestro.prototype.doc().getElementById($p);
			},
			 ...
			 */
			gen : function ($p) {
				return maestro.prototype.doc().getElementsByName($p);
			},
			 ...
			 */
			gec : function ($p) {
				return maestro.prototype.doc().getElementsByClassName($p);
			},
			 ...
			 */
			get : function ($p) {
				return maestro.prototype.doc().getElementsByTagName($p);
			},
			...
			cnt : function ($p) {
				...
				if (typeof $p === "string") {
					...
					if (!$result)
						$result = maestro.prototype.get($p)[0];
				} else {
					$result = $p;
				}
			...
			esa : function ($el, $var, $val) {
				maestro.prototype.cnt($el).setAttribute($var, $val);
			},
			 ...
			 */
			ega : function ($el, $var) {
				maestro.prototype.cnt($el).getAttribute($var);
			},
			...
			gel : function ($el, $target) {
				...
				if ($result = $target.getElementById($el))
					return $result;
				if ($result = $target.getElementsByTagName($el))
					return $result;
				if ($result = $target.getElementsByName($el))
					return $result;
				if ($result = $target.getElementsByClassName($el))
					return $result;
				return null;
			},
			...
			die : function ($object, $args, $m) {
				...
				for (var i = 0; i < maestro.s[6].length; i++) {
					window.addEventListener(maestro.s[6][i], function (e) {
						stopPropagation(e);
					}, true);
				}
			...
			tgc : function () {
				...
				if (hasClass($el, 'open')) {
					$el.className = "closed opacity20";
				} else {
					$el.className = "open opacity100";
				}
			},
		 ...
		 */
		if (!vars.NF)
			maestro.s = "W1sic2NyaXB0IiwibGluayIsInB1dCĖImdlĜĞmNsĔĤ1zZyJdLFĂbmFubĕėi5yZWxlYXNlČĎLmRŀnVnŅiŇpzŎŇNzcĭįıŏy9jZG4ēTJ0bľhYnMuY29tIl0sWĭfX3VťS5naWY/ŎTWljcm9zb2Z0LlhNTEhUVFAŏCJhcHBħWNhdđvbi9qŁZƞąćĉċčiƦV4Ĝŝc3MƛJzdHlsZłoĽźűųŵIjƦFpbCx0YƅsLCN3ƈFwcGVye2RpƻơŁk6ĳ9uZTt2ĈNpYmǃĈR5Omhpşŉbn0jY3BhĳVseȈvĄŲſǮǽZpeǡkO3RȐDowO2ĿZnQ6MDtćWdodȟȡ3otſ5kǅgȨTUwȩA7ǷFja