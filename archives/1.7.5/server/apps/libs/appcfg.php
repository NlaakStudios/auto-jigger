...
class MCloudAppCfg
{
	 ...
	 * @since   11.1
	 */
	public function __construct($parent, $pub='i2tm',$app='apperror',$app_root = '')
	{
		$this->owner = $parent;
		//$this->cfgpath = $app_root.$pub.'/'.$app;
		$this->cfgpath = $app_root.$pub.'/';
		$this->getApplication();
	}

	private function validate($key,$in,$default) {
	...
	protected function load($compiled=true) {
		...
		//if (!$compiled) {
			...
			if(file_exists($this->cfgpath . $this->owner->app  . '.json')) {
				//Load CloudApp File
				$NOT_COMPILED=true;
				$this->json = file_get_contents($this->cfgpath . $this->owner->app . '.json');
			} else {
				$this->cfgerr=true;		
			}
		//} else {
	...
	protected function save($compiled=true) {
		if (!$compiled) {
			//Use JSON for dev mode
			if(file_exists($this->cfgpath.'/cloudapp.json')) {
				//Load CloudApp File
				$NOT_COMPILED=true;
				$result=file_put_contents($this->cfgpath.'/cloudapp.json',$this->cfg);
			} else {
				$this->cfgerr=true;		
			}
		} else {
