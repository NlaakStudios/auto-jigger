...
$VENDOR='../cdn/vendor/';define('TEMPLATES', '../cdn/static/shared/templates/');define('LAYOUTS', '../cdn/static/shared/layouts/');$TIMESTAMP=strftime("%Y.%m.%d.%H%M%S"); //2014.01.01.123456
...
{	
    ...
    public function __construct()
    {
		...
		if ($this->devMode) {			ini_set('track_errors', 1);		} else {			ini_set('track_errors', 0);					}		
	...
	private function processBody() {
		...
		if (isset($this->doc->cfg['app']['template'])) {
			...
			$template=($template=='')?'default':$template;			$file = TEMPLATES.$template.'.html';
			//Load template HTML
