...
/**
 *
 * @class $m.df.BaseImage
 */
$m.df.BaseImage = $m.df.Base.extend('$m.df.BaseImage',
    {},
    {
 ...
 * @class $m.df.Image
 */
$m.df.Image = $m.df.BaseImage.extend('$m.df.Image',
    /** @lends $m.df.Image */
    {},
    /** @lends $m.df.Image.prototype */
    {
        ...
        /**
         * Constructs a new $m.df.Image. If the nldr.loader has already started then the image will be
        ...
        init:function (name, src, onLoadCallback, onErrorCallback)
        {
            ...
            this.name = name;
            this.src = nldr.getPath('images',true);
            ...
            if (nldr.loader.started) // load now if the loader has already been started
                this.load();
        },
         ...
         * @return {$m.df.Image} This image object
         */
...
/**
 * A wrapper around a Canvas element supporting the same API as $m.df.Image.  It can
 * be used to render or pre-render graphics that can be used in APIs that expect an
 * Image object.
 * @class $m.df.CanvasImage
 */
$m.df.CanvasImage = $m.df.BaseImage.extend('$m.df.CanvasImage', {},
    {
...
$m.df.ImageTools = $m.df.Base.extend('$m.df.ImageTools',
    {
        /**
         ...
         * @return {$m.df.CanvasImage} A new $m.df.CanvasImage with the rotations
         */
        rotate:function (image, w, h, directions)
        {
            ...
            return new $m.df.CanvasImage(resultCanvas);
        }
 ...
 * @class $m.df.Subimage
 */
$m.df.Subimage = $m.df.BaseImage.extend('$m.df.Subimage', {},
    {
      ...
      init:function(baseImage, config) {
        ...
        this.loaded = true;
        if(!$m.df.valid(baseImage)) throw new Error('Subimage without base image!');
        this.sourceX = $m.df.checked(config.x, 0);
        this.sourceY = $m.df.checked(config.y, 0);
        this.width = $m.df.checked(config.w, 0);
        this.height = $m.df.checked(config.h, 0);
        this.scaleX = $m.df.checked(config.scaleX, 1.0);
        this.scaleY = $m.df.checked(config.scaleY, 1.0);
        this.alpha = $m.df.checked(config.alpha, 1.0);
        if(!baseImage.loaded) console.log("Warning: Subimage of not-loaded image");
        this.compositeOperation = $m.df.checked(config.compositeOperation, null);
      },
