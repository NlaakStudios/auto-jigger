 ...
 * @class $m.df.components.Joint
 */
$m.df.components.Joint = $m.df.components.Component.extend('$m.df.components.Joint',
    /** @lends $m.df.components.Joint */
    {
        /**
         * Construct (or acquires) a new component with the options supplied
         * @param {$m.df.Entity} options.attachTo Entity to attach the joint to
         * @param {$m.df.Dim} options.offset Dim x, y of the pixel offset the joint is relative to the source entity
         * @param {$m.df.Dim} options.attachmentOffset Dim x, y of the offset of the joint on the attached entity
         * @param {Number} options.distance How long the joint is
         * @param {$m.df.JointType} options.type Type of joint ($m.df.JointType.WELD, $m.df.JointType.REVOLUTE, $m.df.JointType.DISTANCE)
         ...
         * @return {$m.df.components.Joint} A configured joint component
         */
    ...
    /** @lends $m.df.components.Joint.prototype */
    {
        ...
        distance:0,
        /** Type of joint -- see $m.df.JointType */
        type:0,
        ...
        init:function (options)
        {
            ...
            this.attachmentOffset = $m.df.Point.create(0, 0);
        ...
        {
            this.attachTo = $m.df.checked(options.attachedTo, null);
            if (options.offset)
                this.offset.setXY($m.df.checked(options.offset.x, 0), $m.df.checked(options.offset.y, 0));
            if (options.attachmentOffset)
                this.attachmentOffset.setXY($m.df.checked(options.attachmentOffset.x, 0), $m.df.checked(options.attachmentOffset.y, 0));

            this.distance = $m.df.checked(options.distance, 0);
            this.type = $m.df.checked(options.type, $m.df.JointType.WELD);

            this.dampingRatio = $m.df.checked(options.dampingRatio, 0);
            this.lowerAngleLimit = $m.df.checked(options.lowerAngleLimit, 0);
            this.upperAngleLimit = $m.df.checked(options.upperAngleLimit, 359);
            this.enableLimit = $m.df.checked(options.enableLimit, false);
            this.maxMotorTorque = $m.df.checked(options.maxMotorTorque, 100);
            this.enableMotor = $m.df.checked(options.enableMotor, false);
            this.motorSpeed = $m.df.checked(options.motorSpeed, 0);
            this.angle = $m.df.checked(options.angle, 0);
        ...
        getAngle:function ()
        {
            return $m.df.Math.radToDeg(this._joint.GetJointAngle());
        },
