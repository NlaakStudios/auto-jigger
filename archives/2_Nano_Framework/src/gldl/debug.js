 ...
 * by the gldl.device system. When onReady is triggered the panel will automatically attach to a canvas element with the
 * id 'debug'.
 * <p>
 * <pre><code>
 ...
 * <p>
 * You can gain access to the debug panel through gldl.device.debugPanel member.
 * @class $m.df.DebugPanel
 */
$m.df.DebugPanel = $m.df.Base('$m.df.DebugPanel',
    {},
    /** @lends $m.df.DebugPanel.prototype */
    {
        x:0,
        y:0,
        panelHeight:0,
        ...
        attach:function (canvasElement)
        {
            ...
            var np = 4;
            // create the graphs
            this.timeGraph = new $m.df.CanvasLineGraph(this.ctx, 'Performance', '', 10,
                [
            ...
            {
                this.memGraph = new $m.df.CanvasLineGraph(this.ctx, 'Memory', 'Memory profiling not available', 0,
                    [
                        {name:'mem used (mb)', color:'#55f'}
                    ], (this.panelWidth / np) + 10, 10, (this.panelWidth / np) - 10, this.panelHeight - 20);
            } else
            {
                this.memGraph = new $m.df.CanvasLineGraph(this.ctx, 'Memory', '', ((console.memory.totalJSHeapSize / 1024 / 1024) * 1.2),
                    [
                        {name:'mem used (mb)', color:'#55f'}
                    ], (this.panelWidth / np) + 10, 10, (this.panelWidth / np) - 10, this.panelHeight - 20);
            }

            this.poolGraph = new $m.df.CanvasLineGraph(this.ctx, 'Pool Size', '', 100,
                [
                    {name:'pooled', color:'#5b1654'}
                ], this.panelWidth - ((this.panelWidth / np) * 2) + 10, 10, (this.panelWidth / np) - 20, this.panelHeight - 20);

            this.entityGraph = new $m.df.CanvasLineGraph(this.ctx, 'Entities', '', 100,
                [
        ...
        update:function (delta)
        {
            ...
            if (this._timeSince > 30)
            {
                this._timeSince = 0;
                if (this.timeGraph != null)
                    this.timeGraph.addLine2(gldl.device.lastProcessMS, gldl.device.lastDrawMS);
                if (this.entityGraph != null)
                    this.entityGraph.addLine1(gldl.device.elementsDrawn);
                if (this.memGraph != null)
                ...
                if (this.poolGraph != null)
                    this.poolGraph.addLine1($m.df.CS2I.Pool.totalPooled);
            }
        },
 ...
 * @class $m.df.CanvasLineGraph
 */
$m.df.CanvasLineGraph = $m.df.Base.extend('$m.df.CanvasLineGraph', {
