 ...
 * of tiles you want to appear in a <a href='$m.df.TileMap'>tile map</a> (and thus a <a href='$m.df.TileLayer'>tile layer</a>).
 * <p>
 * To construct a tile set, use a spritesheet containing the tile images you want to use:
 * <pre><code>
 * var tileSet = new $m.df.TileSet(tileSheet);
 * </code></pre>
 ...
 *       // climb
 * </code></pre>
 * For convenience, you should probably just use the tileHasProperty method in the <a href='$m.df.TileMap'>$m.df.TileMap</a>
 * class.
 * <pre><code>
 ...
 * @class $m.df.TileSet
 */
$m.df.TileSet = $m.df.Base.extend('$m.df.TileSet',
  {},
  /** @lends $m.df.TileSet.prototype */
  {
    /** sprite sheet used for tiles */
    tileSpriteSheet: null,
    /** $m.df.Hashmap of properties */
    props: null,
    drawOffsetX: 0,
    drawOffsetY: 0,
     ...
     * @param {$m.df.SpriteSheet} spriteSheet Sprite sheet to use for tile images
    ...
    {
      this.tileSpriteSheet = $m.df.checked(spriteSheet, null);
      if (this.tileSpriteSheet)
      {
        this.props = new Array(spriteSheet.totalFrames);
        for (var i = 0; i < this.props.length; i++)
        {
          this.props[i] = new $m.df.Hashmap();
        }
      }

      this.drawOffsetX = $m.df.checked(drawOffsetX, 0);
      this.drawOffsetY = $m.df.checked(drawOffsetY, 0);
      this.idOffset = $m.df.checked(idOffset, 0);
    },
    ...
    drawTile: function (ctx, tileNumber, x, y)
    {
      ...
      this.tileSpriteSheet.drawFrame(
        ...
        $m.df.Math.floor(tileNumber / this.tileSpriteSheet.framesWide),
     ...
     * @return {$m.df.Hashmap} Hashmap of the properties
     */
