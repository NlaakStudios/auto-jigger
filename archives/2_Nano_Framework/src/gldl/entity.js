 ...
 * <p>
 * To add an entity to a game you must place it within an <a href='$m.df.EntityLayer'>entity layer</a>.
 * <p>
 * <pre><code>
 ...
 * <pre><code>
 * var newEntity = $m.df.Entity.create(entityLayer);
 * </code></pre>
 * <h5>Components</h5>
 ...
 * box.addComponent( $m.df.components.Spatial.create({ x:100, y: 100, w:50, h:50 }) );
 *
 * // add a red rectangle
 * box.addComponent( $m.df.components.Rect.create({ color:'#ff2222' }) );
 * </code></pre>
 * <h5>Tagging</h5>
 ...
 * <pre><code>
 * entityLayer.entityManager.getTagged('enemy'); // return a $m.df.LinkedList
 * </code></pre>
 ...
 * @class $m.df.Entity
 */
$m.df.Entity = $m.df.Pooled.extend('$m.df.Entity',
    /** @lends $m.df.Entity */
    {
        /**
         * Constructs an entity by acquiring it from the object pool
         * @param {$m.df.Layer} layer Layer the entity should be added to
         * @return {$m.df.Entity} A $m.df.Entity
         */
        create: function(layer)
        {
            ...
            if (!layer.Class.isA('$m.df.EntityLayer')) throw 'Entities can only be placed on $m.df.EntityLayer objects';
            n.config(layer);
            return n;
        }
    },
    /** @lends $m.df.Entity.prototype */
    {
        ...
        init: function(layer)
        {
            ...
            this.layer = null;
            this.active = true;
            this._componentCache = new $m.df.Hashmap();
            if ($m.df.valid(layer))
                this.config(layer);
        },
         ...
         * @param {$m.df.EntityLayer} layer Layer to add the entity too
         */
         ...
         * @return {$m.df.components.Component} Component that was added
         */
         ...
         * @return {$m.df.components.Component} The component matching the type
         */
         ...
         * @return {$m.df.Hashtable} A hashtable of component objects keyed by component type
         */
 ...
 * @class $m.df.EntityFactory
 */
$m.df.EntityFactory = $m.df.Base.extend('$m.df.EntityFactory',
    { },
    {
        /**
         * Called by the entity loader
         * @param {$m.df.Layer} layer Layer the entity should be placed on
         * @param {String} type String type of the entity to create
         * @param {Number} x X position
         * @param {Number} y Y position
         * @param {Number} dir Facing angle
         * @param {Object} shape Collision shape (either $m.df.Dim for rectangle or $m.df.Poly for polygons)
