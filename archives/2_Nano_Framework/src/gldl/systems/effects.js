 ...
 * @module GLDL
 * @submodule Systems
 * @class $m.df.systems.Effects
 */
$m.df.systems.Effects = $m.df.systems.EntitySystem.extend('$m.df.systems.Effects',
    /** @lends $m.df.systems.Effects */
    {
        FadeState:
        {
            NOT_STARTED: 0,
    ...
    /** @lends $m.df.systems.Effects.prototype */
    {
        /**
            ...
            while (next)
            {
                ...
                if (entity.active)
                {
                    ...
                    if (fade && fade.active)
                    {
                        ...
                        if (!alpha)
                            alpha = entity.addComponent($m.df.components.Alpha.create({}));
                    ...
                    if (spin && spin.spinning && spin.active)
                    {
                        ...
                        var a = spin.rate / gldl.device.elapsed;
                        ...
                        spin.spinSoFar += a;
                        spatial.setDir( $m.df.Math.rotate(spatial.getDir(), spin.clockwise ? a : -a));
                    }
                    ...
                    if (scale && scale.scaling && scale.active)
                    {
                        ...
                        var sx = scale.growX / gldl.device.elapsed;
                        var sy = scale.growY / gldl.device.elapsed;
        ...
        _fade: function(alpha, fader)
        {
            ...
            switch (fader.state)
            {
                ...
                case this.Class.FadeState.FADING_IN:
                    alpha.addAlpha((gldl.device.elapsed * (100 / fader.timeLimit)) / 100);
                    if (timeSinceStart > fader.timeLimit)
                    {
                        fader.timeLimit = fader.holdTime;
                        fader.startTime = gldl.device.now;
                ...
                case this.Class.FadeState.FADING_OUT:
                    ...
                    {
                        alpha.subAlpha((gldl.device.elapsed * (100 / fader.timeLimit)) / 100);
                    }
