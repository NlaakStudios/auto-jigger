 ...
 * @module GLDL
 * @submodule Systems
 * @class $m.df.systems.Layout
 */
$m.df.systems.Layout = $m.df.systems.EntitySystem.extend('$m.df.systems.Layout',
    /** @lends $m.df.systems.Layout */
    {},
    /** @lends $m.df.systems.Layout.prototype */
    {
        ...
        init: function(options)
        {
            this._super( [ 'layout' ] );
            this.margin = {};
            if ($m.df.checked(options) && $m.df.checked(options.margin))
            {
        ...
        doLayout: function()
        {
            var layouts = new $m.df.HashList(); // a list for each of the anchors
            ...
            while (next)
            {
                ...
                if (!spatial)
                    entity.addComponent( $m.df.components.Spatial({}) );
