 ...
 * <a href='$m.df.Scene'>$m.df.Scene</a> which automatically handles calling the process and draw methods.
 * <p>
 * You can use the $m.df.Layer class to create your own custom layers, though typically <a href='$m.df.TileLayer'>
 * $m.df.TileLayer</a> and <a href='$m.df.EntityLayer'>$m.df.EntityLayer</a> should be used for typical game levels.
 * <p>
 * To create your own custom level, derive from $m.df.Level and override the draw and process methods:
 * <pre><code>
 * // construct a scene and add it to the game
 * myGameScene = new GameScene();
 *
 * // create your own layer class
 * MyLayer = $m.df.Layer.extend('MyLayer',
 * {},
 * {
 ...
 *    {
 *       // draw a rectangle
 *       gldl.device.ctx.fillStyle = '#fff';
 *       gldl.device.ctx.fillRect(0, 0, 10, 10);
 *    }
 *
 ...
 *    gldl.device.ctx.fillStyle = '#fff';
 *    gldl.device.ctx.fillRect(this.screenX(100), this.screenY(100), 10, 10);
 * }
 *
 * // override process to update or change things
 * process:function()
 ...
 * @class $m.df.Layer
 */
$m.df.Layer = $m.df.Base.extend('$m.df.Layer', {},
  /** @lends $m.df.Layer.prototype */
  {
    /** Name of the layer */
    name: null,
    ...
    init: function (name, zIndex)
    {
      ...
      this.name = name;
      this.origin = $m.df.Point.create(0, 0);
      this._worldPos = $m.df.Point.create(0, 0);
      this._screenPos = $m.df.Point.create(0, 0);
      this.zIndex = $m.df.checked(zIndex, 0);
     ...
     * @param {$m.df.Point} pos World position of this layer (cached, so you don't need to release it)
     * @param {$m.df.Point} [returnPos] Optional return point (so you can pass in a point to be set)
     */
     ...
     * @param {$m.df.Layer} trackLayer Layer to track
    ...
    setOriginTrack: function (trackLayer, xRatio, yRatio)
    {
      ...
      this.originTrackXRatio = $m.df.checked(xRatio, 1);
      this.originTrackYRatio = $m.df.checked(yRatio, 1);
    },
     ...
     * @param {$m.df.Base} uiTarget the uiTarget where the action occurred
     */
