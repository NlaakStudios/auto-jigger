 ...
 * string in the form of #RRGGBB. (For alpha see the $m.df.component.Alpha and $m.df.components.Fade).
 * <p>
 *     Create a color using either new:
 * <pre><code>
 * var color = new $m.df.Color([255, 0, 0]); // super red
 * var color2 = new $m.df.Color('#00FF00'); // super green
 * </code></pre>
 * <p>
 *     Or, as a pooled object:
 * <pre><code>
 * var color = $m.df.Color.create([255, 0, 0]); // super red
 * </code></pre>
 * @class $m.df.Color
 * @uses $m.df.CS2I.Pooled
 */
$m.df.Color = $m.df.CS2I.Pooled.extend('$m.df.Color',
  /** @lends $m.df.Color */
  {
    /**
  ...
  /** @lends $m.df.Color.prototype */
  {
    ...
    /**
     * Constructs a new color using the passed in color string. Used if you call new $m.df.Color, but typically
     * you should be using $m.df.Color.create as this is a pooled class
     * @param {Array|String} color Can either be a string in the form of #RRGGBB or an array of 3 numbers representing red,
     * green, blue levels, i.e. full red is [255, 0, 0]
     */
    init: function (color)
    ...
    setRed: function (r)
    {
      this.rgb[0] = $m.df.Math.limit(r, 0, 255);
      this._updateColorCache();
    },
    ...
    addRed: function (r)
    {
      this.rgb[0] = $m.df.Math.limitAdd(this.rgb[0], r, 0, 255);
      this._updateColorCache();
    },
    ...
    setGreen: function (g)
    {
      this.rgb[1] = $m.df.Math.limit(g, 0, 255);
      this._updateColorCache();
    },
    ...
    addGreen: function (g)
    {
      this.rgb[1] = $m.df.Math.limitAdd(this.rgb[1], g, 0, 255);
      this._updateColorCache();
    },
    ...
    setBlue: function (b)
    {
      this.rgb[2] = $m.df.Math.limit(b, 0, 255);
      this._updateColorCache();
    },
    ...
    addBlue: function (b)
    {
      this.rgb[2] = $m.df.Math.limitAdd(this.rgb[2], b, 0, 255);
      this._updateColorCache();
    },
