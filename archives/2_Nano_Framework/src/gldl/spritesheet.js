 ...
 * <p>
 * To create an on-screen sprite you need to setup a sprite sheet template, then construct a $m.df.Sprite object
 * using the sheet.
 * <p>
 ...
 * var zombieImage = gldl.device.loader.get('zombie').resource;
 *
 * // create the spritesheet
 * var zombieSpriteSheet = new $m.df.SpriteSheet(
 ...
 * To make an actual sprite you can draw on the screen, use the <a href='$m.df.Sprite'>$m.df.Sprite</a> class.
 * @class $m.df.SpriteSheet
 */
$m.df.SpriteSheet = $m.df.Base.extend('$m.df.SpriteSheet',
  /** @lends $m.df.SpriteSheet */
  {},
  /** @lends $m.df.SpriteSheet.prototype */
  {
     ...
     * @param {$m.df.Image} options.image Image to use for the sheet
    ...
    init: function (options)
    {
      ...
      this.sourceX = $m.df.checked(options.sourceX, 0);
      this.sourceY = $m.df.checked(options.sourceY, 0);
      this.frameOffsetX = $m.df.checked(options.frameOffsetX, 0);
      this.frameOffsetY = $m.df.checked(options.frameOffsetY, 0);
      this.alpha = $m.df.checked(options.alpha, 1);
      this.useRotation = $m.df.checked(options.useRotation, true);

      this.options = options; //DELME

      this.animations = new $m.df.Hashtable();
      if('frames' in options)
      {
        this.frames = options.frames;
        this.totalFrames = options.frames.length;
      }
      else if($m.df.valid(image))
      {
    ...
    {
      if (!$m.df.valid(options.name)) throw "Animation requires a name for reference";

      options.frameX = $m.df.checked(options.frameX, 0);
      options.frameY = $m.df.checked(options.frameY, 0);
      options.directions = $m.df.checked(options.directions, 1);
      options.time = $m.df.checked(options.time, 1000);
      options.loops = $m.df.checked(options.loops, 0);
      options.holdOnEnd = $m.df.checked(options.holdOnEnd, false);
      options.dirAcross = $m.df.checked(options.dirAcross, false);
      options.scaleX = $m.df.checked(options.scaleX, 1);
      options.scaleY = $m.df.checked(options.scaleY, 1);
      options.offsetX = $m.df.checked(options.offsetX, 0);
      options.offsetY = $m.df.checked(options.offsetY, 0);
      options.framesWide = $m.df.checked(options.framesWide, this.framesWide);
      options.framesHigh = $m.df.checked(options.framesHigh, this.framesHigh);
      options.frameCount = $m.df.checked(options.frameCount, 0);

      // no frames specified, create the frames array automagically
      if (!$m.df.valid(options.frames))
      {
    ...
    setAnimation: function (state, name, speedOffset)
    {
      ...
      state.held = false;
      state.animSpeedOffset = $m.df.checked(speedOffset, 0);
    },
     ...
     * @param {$m.df.Sprite} state Sprite to draw
    ...
    drawFrame: function (ctx, frameX, frameY, x, y, angle)
    {
      ...
      frameImage.draw(ctx,
        this.sourceX + frameSourceX,
        this.sourceY + frameSourceY,
        $m.df.Math.round(x) + this.frameOffsetX - frameRegX,
        $m.df.Math.round(y) + this.frameOffsetY - frameRegY,
    ...
    {
      return this.frames[$m.df.checked(x, 0) + ($m.df.checked(y, 0) * this.framesWide)];
    },
    ...
    getFrameAsImage: function(x, y)
    {
      ...
      return new $m.df.Subimage(frame[4], {
        x:frame[0],
        y:frame[1],
        w:frame[2],
        h:frame[3]
     ...
     * @param {$m.df.Sprite} state Sprite to update
