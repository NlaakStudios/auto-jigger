 ...
 * methods/members are accessed using $m.df.Math.
 * @class $m.df.Math
 */
$m.df.Math = $m.df.Base('$m.df.Math',
  /** @lends $m.df.Math */
  {
     ...
     * @return {Number} A random number between (and including) the range
     */
    rand: function (min, max)
    {
      return $m.df.Math.round(($m.df.Math.random() * (max - min)) + min);
    },
    ...
    {
      return ($m.df.Math.random() * (max - min)) + min;
    },
     ...
     * @param angle
     * @return {Number} an angle between
     */
    ...
    {
      return (radians * $m.df.Math.RADIAN_TO_DEGREE);
    },
    ...
    {
      return degrees * $m.df.Math.DEGREE_TO_RADIAN;
    },
    ...
    {
      // angle to vector
      var a = $m.df.Math.radToDeg(Math.atan2(y, x));
      if (a < 0) a += 360;
      return a;
    },

    /**
     * Gives you the x, y vector of a given angle in degrees. This method creates a $m.df.Point which you should
     * release after use
     * @param {Number} angle Angle in degrees
     * @return {$m.df.Point} A newly acquired $m.df.Point with the vector.
     */
    vectorFromAngle: function (angle)
    {
      var vx = Math.cos($m.df.Math.degToRad(angle));
      var vy = Math.sin($m.df.Math.degToRad(angle));
      return $m.df.Point.create(vx, vy);
    },
    ...
    {
      if (!$m.df.Math.isPointInRect(x, y, rx, ry, rw, rh)) return false;
      if (!$m.df.Math.isPointInRect(x + w, y, rx, ry, rw, rh)) return false;
      if (!$m.df.Math.isPointInRect(x, y + h, rx, ry, rw, rh)) return false;
      return $m.df.Math.isPointInRect(x + w, y + h, rx, ry, rw, rh);
    },
 ...
 * @class $m.df.Rect
 */
$m.df.Rect = $m.df.Pooled('$m.df.Rect',
  /** @lends $m.df.Rect */
  {
    /**
     ...
     * @return {$m.df.Rect} A new rectangle (acquired from the free object pool}
     */
  ...
  /** @lends $m.df.Rect.prototype */
  {
     ...
     * @param {$m.df.Point} p A $m.df.point (or any object with a .x and .y property
    ...
    overlaps: function (rx, ry, rw, rh, dir)
    {
      ...
      if ($m.df.valid(dir) && dir != 0)
      {
        // calculate using a rotated rectangle
        var s = Math.sin($m.df.Math.degToRad(dir));
        var c = Math.cos($m.df.Math.degToRad(dir));
        if (s < 0) s = -s;
        if (c < 0) c = -c;
 ...
 * @class $m.df.Point
 */
$m.df.Point = $m.df.Pooled('$m.df.Point',
  /** @lends $m.df.Point */
  {
    /**
     ...
     * @return {$m.df.Point} A shiny new point
     */
    create: function (x, y)
    {
      var n = this._super();
  ...
  /** @lends $m.df.Point.prototype */
  {
     ...
     * @param {$m.df.Point} other Other point to test against
     */
     ...
     * @param {$m.df.Point} p The other point to match
     */
    match: function (p)
    {
      this.x = p.x;
     ...
     * @return {$m.df.Point} New point from the pool
     */
    clone: function()
    {
      return $m.df.Point.create(this.x, this.y);
    },
    ...
    /**
     * Adds to the point
     * @param {Number|$m.df.Point} xOrPoint Amount to add to x (or alternatively a $m.df.Point to add)
     ...
     * @param {$m.df.Point} p Another point
    ...
    {
      this.x += distance * Math.cos($m.df.Math.degToRad(dir));
      this.y += distance * Math.sin($m.df.Math.degToRad(dir));
      return this;
    },
     ...
     * @param {$m.df.Point} to Ending position
 ...
 * @class $m.df.Poly
 */
$m.df.Poly = $m.df.Pooled('$m.df.Poly',
  /** @lends $m.df.Poly */
  {
  ...
  /** @lends $m.df.Poly.prototype */
  {
    ...
    init: function (x, y, points)
    {
      ...
      this.y = y;
      this.points = points;
      this._boundingRect = $m.df.Rect.create(0, 0, 0, 0);
    ...
    {
      if(!$m.df.valid(this.points) || !this.points.length)
        return;
