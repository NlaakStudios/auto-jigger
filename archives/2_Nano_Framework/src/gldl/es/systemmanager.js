 ...
 */
$m.df.SystemManager = $m.df.Base.extend('$m.df.SystemManager',
    /** @lends $m.df.SystemManager */
    {},
    /** @lends $m.df.SystemManager.prototype */
    {
        /** $m.df.LinkedList of systems */
        systems:null,
        ...
        init:function (layer)
        {
            this.systems = new $m.df.LinkedList();
            this.systemsByComponentType = new $m.df.Hashtable();
            this.layer = layer;
        },

        /**
         * Adds a system to the system manager
         * @param {$m.df.systems.System} system System to add
         */
        add:function (system)
        {
            ...
            if (!$m.df.valid(system.componentTypes))
         ...
         * @param {$m.df.systems.System} system System to remove
         */
         ...
         * @return {$m.df.LinkedList} A linked list of the systems that have the given component type
         */
        ...
        processAll:function ()
        {
            ...
            while (next)
            {
                if (next.obj.delay == 0 || (gldl.device.now - next.obj._lastRun > next.obj.delay))
                {
                    next.obj.processAll();
                    if (next.obj.delay != 0)
                        next.obj._lastRun = gldl.device.now;
                }
                next = next.next();
            }
        },
