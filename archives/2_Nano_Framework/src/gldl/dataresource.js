 ...
 * <p>
 * To load a resource, use the $m.df.Loader to add a resource:
 * <pre><code>
 * gldl.device.loader.add(new $m.df.DataResource('level1', 'data/level1.tmx'));
 * </code></pre>
 * <p>
 * Once you have the resource loaded you can access the contents of the resource using the data member:
 * <pre><code>
 * var xmlData = gldl.device.loader.get('level1').resource.data;
 * </code></pre>
 * <p>
 ...
 *    // dataResource.data
 * }
 * gldl.device.loader.add(new $m.df.DataResource('level1', 'data/level1.tmx', onLevelDataLoaded));
 * </code></pre>
 * <p>
 * The Scrollia demo game has an example using that loads the level.tmx file from the editor as a data resource which
 * is passed to $m.df.Scene to construct entities and layers.
 * @class $m.df.DataResource
 * @uses $m.df.Base
 */
$m.df.DataResource = $m.df.Base.extend('$m.df.DataResource',
    {},
    /** @lends $m.df.DataResource.prototype */
    {
        ...
        {
            this._super();
            this.src = gldl.device.loader.makeUrl(src);
            this.name = name;
         ...
         * the $m.df.Loader will automatically call load when it starts.
