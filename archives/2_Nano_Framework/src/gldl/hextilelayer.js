 ...
 * @class $m.df.HexTileLayer
 */
$m.df.HexTileLayer = $m.df.TileLayer.extend('$m.df.HexTileLayer',
  /** @lends $m.df.HexTileLayer */
  {
  },
  /** @lends $m.df.HexTileLayer.prototype */
  {
    _hexSide: 0,
    _halfHexSide: 0,
    _yInc: 0,
    ...
    init: function (name, tileMap, tileSet)
    {
      ...
      // temps
      this._worldPosTemp = $m.df.Point.create(0, 0);
    },
    ...
    {
      if (!returnPos) returnPos = $m.df.Point.create(0,0);
      ...
      for (var hy = ty, c = th + ty; hy < c + 1; hy++)
      {
        ...
        for (var hx = tx, d = tw + tx; hx < d; hx++)
        {
          ...
          if (tileType >= 0)  // -1 means no tile
          {
            ...
            if (this._debug)
            {
              gldl.device.ctx.fillStyle = '#888';
              gldl.device.ctx.font = 'Arial 7pt';
              gldl.device.ctx.fillText('' + hx + ',' + hy,
