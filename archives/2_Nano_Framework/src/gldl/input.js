 ...
 * engine will automatically construct a single, global input class which is accessible via the global gldl.device.input.
 * <p>
 ...
 * the <a href='$m.df.Game'>game</a>, <a href='$m.df.Layer'>layer</a> or <a href='$m.df.Scene'>scene</a> classes for more
 ...
 * process:function ()
 * {
 *    if (gldl.device.input.isInputState(this, 'moving left'))
 *       // move the player left
 * }
 * </code></pre>
 * You can see an example of both input actions and states in the Asteroids sample game.
 * <p>
 * Rather than using this class directly, you can also use the <a href='$m.df.components.Input'>input component</a>
 * and <a href='$m.df.systems.Input'>system</a> which lets you bind input to an entity as a component.
 * @class $m.df.Input
 */
$m.df.Input = $m.df.Base('$m.df.Input',
  /** @lends $m.df.Input */
  {
     ...
     * @param {$m.df.Point} [pos] Position object to set. Leave out to have a new (pooled) point returned
     */
    getEventPosition: function (e, pos)
    {
      if (this._eventPos == null)
        this._eventPos = $m.df.Point.create(0, 0);

      var r = pos;
      if (!$m.df.Tools.isValid(pos))
        r = this._eventPos;
  ...
  /** @lends $m.df.Input.prototype */
  {
    ...
    {
      this._super();
      this.stateBindings = new $m.df.Hashtable();
      this.states = new $m.df.Hashtable();
      this.actionBindings = new $m.df.Hashtable();
      this.mousePos = $m.df.Point.create(0, 0);
      this.deviceOrientation = $m.df.Point.create(0, 0);
    },
     ...
     * @param {String} input The name of the input, i.e. 'LEFT' (see $m.df.InputType)
    ...
    bindState: function (obj, stateName, input, uiTarget)
    {
      ...
      if (!$m.df.InputType.getCode(input))
        throw "Unknown input code " + input + ' - see $m.df.InputType for a list of input names';
      ...
      if ($m.df.InputType.isPositional($m.df.InputType.getCode(input)))
      ...
      for (var b = 0; b < bindings.length; b++)
      {
        ...
        for (var i = 0; i < bindingSet.length; i++)
        {
          ...
          if (binding.object == obj)
          {
            ...
            state.on = false;
            state.event = null;
            if ($m.df.InputType.isPositional(binding.input))
              $m.df.tools.arrayRemove(this._positionals, binding);
          }
        }
      }
    },
     ...
     * gldl.device.input.bindAction(menuLayer, 'new game', 'MOUSE_BUTTON_LEFT_DOWN', menuOption);
     * </code></pre>
     ...
     *
     * @param {$m.df.Base} obj The entity, layer or scene to bind this action to (must implement onAction)
     * @param {String} actionName The name of the action, e.g. 'FIRE' or 'JUMP'
     * @param {String} input The input code as a string
     * @param {$m.df.Base} [uiTarget] An optional element to limit the input to only within the bounds of the element (must
    ...
    {
      var keyName = $m.df.InputType.getName(eventCode);
      ...
      for (var i = 0; i < bindingSet.length; i++)
      {
        ...
        if (!obj.isActive || obj.isActive())
        {
          ...
          if ($m.df.InputType.isPositional(eventCode))
          {
            var pos = this.Class.getEventPosition(event);

            if ($m.df.InputType.isDeviceOrientation(eventCode))
            {
            ...
            var er = null;
            if ($m.df.valid(binding.uiTarget))
    ...
    {
      // touch input
      var eventSource = gldl.device.isCocoonJS ? document.body : gldl.device.canvas;
    ...
    _checkPositional: function (moveEvent)
    {
      ...
      for (var i = 0; i < this._positionals.length; i++)
      {
        ...
        if (moveEvent.type == 'mousemove' && $m.df.InputType.isTouch($m.df.InputType.getCode(binding.input)))
          continue;

        if (moveEvent.type == 'touchmove' && !$m.df.InputType.isTouch($m.df.InputType.getCode(binding.input)))
          continue;

        if ($m.df.InputType.getCode(binding.input) == $m.df.InputType.MOUSE_BUTTON_LEFT_UP ||
          $m.df.InputType.getCode(binding.input) == $m.df.InputType.MOUSE_BUTTON_LEFT_DOWN ||
          $m.df.InputType.getCode(binding.input) == $m.df.InputType.MOUSE_BUTTON_RIGHT_UP ||
          $m.df.InputType.getCode(binding.input) == $m.df.InputType.MOUSE_BUTTON_RIGHT_DOWN
          )
          continue;
    ...
    _changeState: function (eventCode, stateOn, event)
    {
      ...
      if (keyName == null)
      {
        ...
        return false;
      } else if (keyName=="ESC" || keyName=="TAB" || keyName=="F5" || keyName=="F11") {
        this.warn("Keycode = " + eventCode + " Pressed, pause game.");
		if (r2wl.launcher!=null) {
			r2wl.launcher.pauseGame();
			gldl.device.game.stopAllSounds();
		}
	  }
    ...
    /**
     * Called by the gldl.device main loop to process any move events received. We only handle events
    ...
    process: function ()
    {
      if (this._lastMouseMove)
      {
        ...
        this.fireAction($m.df.InputType.MOUSE_MOVE, this._lastMouseMove);
      ...
      {
        this._changeState($m.df.InputType.DEVICE_ORIENTATION, true, this._lastDeviceOrientation);
        this.fireAction($m.df.InputType.DEVICE_ORIENTATION, this._lastDeviceOrientation);
    ...
    _touchStart: function (event)
    {
      ...
      {
        this._changeState($m.df.InputType.TOUCH, true, event.touches[i]);
        this.fireAction($m.df.InputType.TOUCH, event.touches[i]);
      }
    ...
    _touchEnd: function (event)
    {
      for (var i = 0, len = event.changedTouches.length; i < len; i++)
      {
        ...
        this._changeState($m.df.InputType.TOUCH_END, false, event.changedTouches[i]);
        this.fireAction($m.df.InputType.TOUCH_END, event.changedTouches[i]);
      }
    },
    ...
    _touchMove: function (event)
    {
      for (var i = 0, len = event.touches.length; i < len; i++)
      {
        ...
        this._changeState($m.df.InputType.TOUCH_MOVE, false, event.changedTouches[i]);
        this.fireAction($m.df.InputType.TOUCH_MOVE, event.touches[i]);
      }
    },
    ...
    _mouseUp: function (event)
    {
      ...
      {
        this._changeState($m.df.InputType.MOUSE_BUTTON_LEFT_DOWN, false, event);
        this._changeState($m.df.InputType.MOUSE_BUTTON_LEFT_UP, true, event);
        this.fireAction($m.df.InputType.MOUSE_BUTTON_LEFT_UP, event);
        this.mouseLeftButtonDown = false;
      } else
      {
        this._changeState($m.df.InputType.MOUSE_BUTTON_RIGHT_DOWN, false, event);
        this._changeState($m.df.InputType.MOUSE_BUTTON_RIGHT_UP, true, event);
        this.fireAction($m.df.InputType.MOUSE_BUTTON_RIGHT_UP, event);
    ...
    _mouseDown: function (event)
    {
      if (event.button == 0 || event.button == 1)
      {
        this._changeState($m.df.InputType.MOUSE_BUTTON_LEFT_DOWN, true, event);
        this._changeState($m.df.InputType.MOUSE_BUTTON_LEFT_UP, false, event);
        this.fireAction($m.df.InputType.MOUSE_BUTTON_LEFT_DOWN, event);
        this.mouseLeftButtonDown = true;
      } else
      {
        this._changeState($m.df.InputType.MOUSE_BUTTON_RIGHT_DOWN, true, event);
        this._changeState($m.df.InputType.MOUSE_BUTTON_RIGHT_UP, false, event);
        this.fireAction($m.df.InputType.MOUSE_BUTTON_RIGHT_DOWN, event);
        this.mouseRightButtonDown = true;
      }
    },
    _mouseDoubleClick: function (event) {
        if (event.button == 0 || event.button == 1) {
            this._changeState($m.df.InputType.MOUSE_BUTTON_LEFT_DBLCICK, true, event);
            this.fireAction($m.df.InputType.MOUSE_BUTTON_LEFT_DBLCICK, event);
        }
        else {
            this._changeState($m.df.InputType.MOUSE_BUTTON_RIGHT_DBLCICK, true, event);
            this.fireAction($m.df.InputType.MOUSE_BUTTON_RIGHT_DBLCICK, event);
        }
    },
    ...
    _mouseWheel: function (event) {
        ...
        if (event.wheelDelta > 0 || event.detail > 0) {
            this.fireAction($m.df.InputType.MOUSE_WHEEL_UP, event);
        }
        else {
            this.fireAction($m.df.InputType.MOUSE_WHEEL_DOWN, event);
...
/**
 * @class $m.df.InputType
 */
$m.df.InputType = $m.df.Base.extend('$m.df.InputType',
  {
    // STATICS
    nameToCode: null,
    codeToName: null,
    ...
    init: function ()
    {
      this.nameToCode = new $m.df.Hashtable();
      this.codeToName = new $m.df.Hashtable();
