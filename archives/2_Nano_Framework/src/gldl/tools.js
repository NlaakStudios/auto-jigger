 ...
 * <p><pre><code>
 * var cleanValue = $m.df.Tools.checked(value, 'default');
 * </code></pre>
 * There are shortcuts for the following common tools functions to make like a little easier:
 * <p><pre><code>
 * $m.df.valid = $m.df.Tools.isValid;
 * $m.df.checked = $m.df.Tools.checked;
 * $m.df.assert = $m.df.Tools.assert;
 * </code></pre>
 * @class $m.df.Tools
 */
$m.df.Tools = $m.df.Base.extend('$m.df.Tools',
    /** @lends $m.df.Tools */
    {
        /**
...
if (!window.btoa) window.btoa = $m.df.Base64.encode
if (!window.atob) window.atob = $m.df.Base64.decode