...
/**
 * <p>
 * $m.df.Device is the primary interface between your game and the underlying hardware. It's a singleton instance
 * that will be constructed automatically and is globally accessible at all times as gldl.device
 * <p>
 * gldl.device will automatically be setup once $m.df.JSLoader has completed loading all required javascipt through a call
 * to gldl.device.boot passing in the Id of the canvas element for your game as well as the name of the game class
 * which gldl.device will then dynamically construct. Typically you do not need to construct your own $m.df.Device, $m.df.start
 * will take care of it for you.
 * @class $m.df.Device
 */
$m.df.Device = $m.df.Base.extend('$m.df.Device',
  { },
  /** @lends $m.df.Device.prototype */
  {
    ...
    // device information
    /** $m.df.Rect of the current screen dimensions */
    ...
    /** $m.df.Input handler global instance */
    input: null,
    ...
    {
		this.devMode = this.showDebug = gldl.devMode;
		if (this.devMode)
			this.info('nano Game Engine v' + $m.df.VERSION + ' starting');
		...
		this.debugPanel = new $m.df.DebugPanel();
		this.input = new $m.df.Input();
		this.loader = new $m.df.Loader();
		this.isAppMobi = (typeof AppMobi != "undefined");
		this.isCocoonJS = navigator.isCocoonJS;

		this.pixelRatio = $m.df.CS2I.Device.pixelRatio;
		this.isiPhone = $m.df.CS2I.Device.isiPhone;
		this.isiPhone4 = $m.df.CS2I.Device.isiPhone4;
		this.isiPad = $m.df.CS2I.Device.isiPad;
		this.isAndroid = $m.df.CS2I.Device.isAndroid;
		this.isTouch = $m.df.CS2I.Device.isTouch;
		this.isiOS = (this.isiPhone || this.isiPad);
		this.device = $m.df.CS2I.Device;

		this.requestAnimFrame = $m.df.CS2I.Device.requestAnimFrame;
		...
		r2wl.launcher.toggle('loader',false,false);
		this.onReady();
    },
    ...
    {
      return $m.df.CS2I.Device.canPlay(format);
    },
    ...
    _calcScreenSize: function ()
    {
      if (this.isAppMobi)
      {
        ...
        this.screen = $m.df.Dim.create(window.innerWidth, window.innerHeight);
	  ...
	  else
      {
        ...
        this.screen = $m.df.Dim.create(this.canvasWidth, this.canvasHeight);
      }
    },
    ...
    onReady: function ()
    {
      ...
      if (this.started || !this.gameClass) return; // check we haven't already started
...
//                this.canvas.height = w;
    ...
    {
      return $m.df.Math.isRectColliding(x, y, w, h, 0, 0, this.canvasWidth, this.canvasHeight);
    },
