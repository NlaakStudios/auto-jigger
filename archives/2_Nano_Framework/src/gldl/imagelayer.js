 ...
 * @class $m.df.ImageLayer
 */
$m.df.ImageLayer = $m.df.Layer.extend('$m.df.ImageLayer',
    {
      loadFromJson:function(scene, info)
      {
        var resource = (info.properties && info.properties.resource) || info.name;
        var image = gldl.device.loader.get(resource).resource;
        var newLayer = new $m.df.ImageLayer(info.name, image);
    ...
    },
    {
       ...
       *
       * @param name Name to use (passed up to $m.df.Layer)
       * @param image Image to use; must implement draw() just like $m.df.Image
      ...
      init:function(name, image, zIndex, opts) {
        ...
        if(arguments.length > 3) {
          this.repeatX = $m.df.checked(opts.repeatX, this.repeatX);
          this.repeatY = $m.df.checked(opts.repeatY, this.repeatY);
        }
      },
      ...
      draw:function() {
        ...
        var cw = gldl.device.canvasWidth;
        var ch = gldl.device.canvasHeight;
        var drawnTimes = 0;
        var ctx = gldl.device.ctx;
