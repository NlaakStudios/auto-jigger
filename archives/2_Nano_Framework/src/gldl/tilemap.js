...
/**
 * <p>
 * A map of tiles using by $m.df.TileLayer to what to draw. See <a href='$m.df.TileLayer'>$m.df.TileLayer</a> for details
 ...
 * @class $m.df.TileMap
 */
$m.df.TileMap = $m.df.Base.extend('$m.df.TileMap',
  /** @lends $m.df.TileMap */
  {
    EMPTY_TILE: -1
  },
  /** @lends $m.df.TileMap.prototype */
  {
    ...
    {
      this.tiles = tiles;
      this.tilesWide = $m.df.Math.round(tilesWide);
      this.tilesHigh = $m.df.Math.round(tilesHigh);

      if($m.df.valid(tileWidth))
        this.tileWidth = tileWidth;
      else if(tileSets.length > 0) // Maybe they don't have any tiles (yet?)
        this.tileWidth = tileSets[0].tileSpriteSheet.frameWidth;

      if($m.df.valid(tileHeight))
    ...
    generate: function (tileType)
    {
      ...
      var t = $m.df.checked(tileType, this.Class.EMPTY_TILE);
    ...
    {
      this.drawTileByIdTo(gldl.device.ctx, tileX, tileY, x, y);
    },
    ...
    loadFromTMX: function (layerXML, tileWidth, tileHeight)
    {
      ...
      if (data.getAttribute('encoding') == 'base64')
      {
        ...
        var decoded = $m.df.Base64.decode(tileData);
