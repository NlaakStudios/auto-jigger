 ...
 *       new $m.df.Sound('shotgun', 'sounds/shotgun', ['ogg', 'mp3'], 5));
 * }
 * </code></pre>
 * <p>
 ...
 * var shotgunSound = nldr.loader.get('shotgun').resource;
 ...
 * @class $m.df.Sound
 */
$m.df.Sound = $m.df.Base.extend('$m.df.Sound', {},
    /** @lends $m.df.Sound.prototype */
    {
        ...
        init:function (name, src, formats, channels, onLoadCallback, onErrorCallback)
        {
            ...
            {
                if (gldl.device.canPlay(formats[i]))
                {
                    //this.src = nldr.loader.makeUrl(src + '.' + formats[i]);
					this.src = nldr.loader.getPath('sounds',true);
                    canplay = true;
        ...
        canPlay: function()
        {
            return (this.loaded && gldl.device.soundEnabled && !this.errored);
        }
