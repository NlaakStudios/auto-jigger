 ...
 * <p>
 * To create a tile layer, first create a <a href='$m.df.TileMap'>tile map</a> containing the tile images and map data:
 * <pre><code>
 * // grab a tile sheet previously added to the resource loader
 * var tileSheet = new $m.df.SpriteSheet(
 *    { image:gldl.device.loader.get('myTiles').resource,
 ...
 * var tileMap = new $m.df.TileMap([new $m.df.TileSet(tileSheet)], 100, 100, 32, 32);
 *
 ...
 * var myTileLayer = new $m.df.TileLayer('my tile layer', true, tileMap);
 * </code></pre>
 * <p>
 * Refer to <a href='$m.df.TileMap'>$m.df.TileMap</a> and <a href='$m.df.TileSet'>$m.df.TileSet</a> for more information on tile
 * graphics and maps.
 * <p>
 ...
 * constructor. Typically this is not used directly; you should use the $m.df.Scene loadFromTMX method for more information
 ...
 * var myTileLayer = new $m.df.TileLayer('my tile layer', false);
 * </code></pre>
 * <p>
 * If you change the tile map, you can use the prerender method to update the cache images.
 * @class $m.df.TileLayer
 */
$m.df.TileLayer = $m.df.Layer.extend('$m.df.TileLayer',
    /** @lends $m.df.TileLayer */
    {
        /**
         * Constructs a tile layer using data from a TMX formatted (XML base 64) data stream
         * @param {$m.df.Scene} scene Scene to add the new layer to
        ...
        loadFromTMX:function (scene, layerXML, tileWidth, tileHeight, tileSets)
        {
            ...
            var newLayer = new $m.df.TileLayer(name, true, null, tileSets);
    ...
    /** @lends $m.df.TileLayer.prototype */
    {
        /** $m.df.TileMap data used for this tile layer */
        tileMap:null,
         ...
         * @param {$m.df.TileMap} [tileMap] Tile map data used for the tile layer
         * @param {$m.df.TileSet} [tileSet] If no tile map is supplied, you can optional provide a tile set and a
         * tile map will be constructed using this tile set
         */
        init:function (name, usePrerendering, tileMap, tileSets)
        {
            this._super(name);
            if ($m.df.valid(tileMap))
                this.tileMap = tileMap;
            else
                this.tileMap = new $m.df.TileMap(tileSets);

            this.usePrerendering = $m.df.checked(usePrerendering, true);
        ...
        /**
         * Set the tile map
         * @param {$m.df.TileMap} tileMap The tile map to set
         */
        ...
        drawTiled:function ()
        {
            ...
            for (var y = ty, c = ty + th; y < c + 1; y++)
            {
                ...
                for (var x = tx, d = tx + tw; x < d; x++)
                {
                    ...
                    {
                        gldl.device.ctx.save();
                        gldl.device.ctx.strokeStyle = '#222222';
                        gldl.device.ctx.strokeRect(this.screenX(x * this.tileMap.tileWidth), this.screenY(y * this.tileMap.tileHeight),
                            this.tileMap.tileWidth, this.tileMap.tileHeight);
                        gldl.device.ctx.restore();
                    }
                }
            }
        },
        ...
        drawPrerendered:function ()
        {
            ...
            for (var cy = startPY; cy < maxPY; cy++)
            {
                for (var cx = startPX; cx < maxPX; cx++)
                {
                   ...
                   if(prerender)
                      gldl.device.ctx.drawImage(prerender,
