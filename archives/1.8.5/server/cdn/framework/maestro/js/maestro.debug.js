...
(function() {
	...
	window.onload=function() {
		...
		M$._I.domLoaded=true;
		dbg('window event onLoaded.');
		M$['addEvents']();
		M$['onResizeComplete']();
		if (typeof M$ != 'undefined') M$.on3PL();
		//W$['M$']=W$['M$'];						
	}; 
	...
	*/
	var Maestro = {
		 ...
		 */			
		on3PL:function(){
			dbg('Load 3rd party APIs.');
			if (typeof this.config=='undefined') {
				dbg('No configuration file found.');
				return;
			}
		...
		start:function() {
			...
			//dbg('Successful, Initializing Maestro in '+this[_I.mode]+' mode.');
			if(location.protocol=='file:') { 
				this.dbg('error','You cannot run document locally. Please access through web server.');
				return false
			}
			...
			//	function(){
			//		dbg('Production.css error')														/**DEBUG**/
			//	});
			//this.lds('script','nano-js', this.PATH.cdn+'maestro/js/production_tail.js', true,false,this.contentLoaded, 
			//	function(){
			//		dbg('Production_tail.js error')													/**DEBUG**/
			//	});

			/* if no config then run in limited supportive mode */
			//if (typeof this.config=='undefined') {
			//	dbg('No configuration found.');
			//	this[_I.mode]='Limited';
			//} else this[_I.mode]='Full';
		...
		parseApplet:function(data,$url) {
			...
			function processCode(tag,source) {
				/**
				 * @private
				 */
				function minimize( _content ) {
					dbg('Minimizing...');/*|DEBUG|*/
					var c = _content;
					c = c.replace( /\/\*(?:(?!\*\/)[\s\S])*\*\/|[\r\n\t]+/g, '' );
				...
				dbg('Processing '+tag+' embeded code...');
				var code,src='';
				code = data.getElementsByTagName(tag.toLowerCase());
				if (code.length>0) {
					for (var i=0;i<code.length;i++) {
						if (tag=='script') {
							if (code[i].outerHTML.indexOf('adsbygoogle')===-1) {
								//process javascript, skip google stuff (just remove it)
					...
					if (src.length>0) {
						var genid='applet_'+data.id+"_"+tag;
						src=minimize(src);
						
						var a=D$.createElement(tag);
						a.id=genid;
						a.innerText=src;
						dbg('Attaching '+tag+' ' +genid+ ' to DOM.');
						if (!H$||!T$){dbg('Head (H$) and Tail (T$) alias not set.');return}
						if (tag=='style') {
							H$.appendChild(a)
							dbg('Attached to head successful.');/*|DEBUG|*/
						} else {
							try {	
								T$.appendChild(a);
								eval(genid.innerHTML);
								dbg('Attached to tail successful.');/*|DEBUG|*/
							}catch (e) {
								dbg('Applet '+genid+' Error: '+e.message);
								return;
							}
								
							/**|applet onUnloaded()|**/
							if (typeof M$[genid].onUnloaded==='function') {
								dbg('Attaching '+genid+':onUnloaded() event.');
								try {	
									a.id.onunload=M$[genid].onUnloaded();
									dbg('onUnloaded successful.');/*|DEBUG|*/
								}catch (e) {
									dbg('Applet '+genid+' Error: '+e.message);
									return;
								}
							...
							/**|applet onLoaded()|**/
							if (typeof M$[genid].onLoaded==='function') {
								dbg('Envoking '+genid+':onLoaded() event.');
								try {	
									M$[genid].onLoaded();
									dbg('onLoaded successful.');/*|DEBUG|*/
								}catch (e) {
									dbg('Applet '+genid+' Error: '+e.message);
									return;
								}
			...
			var parser = new DOMParser(),
				...
				tags = doc.getElementsByTagName('ins'),
				applet = tags[0];	
			
			if (typeof data ==='string') data=applet;
			
			if (data&&data['tagName']){
				dbg('Valid Element found, Tag name is defined.');
				if (data['tagName']==='INS' && data.dataset.component && data.dataset.component==='applet') {
					dbg('Tag type is INS - processing applet...');
					...
					dbg('target element ['+M$['VIEWPORT']+'].');

					dbg('Applet ' + data.id + ' by ' + data.dataset.publisher +'...');/*|DEBUG|*/
					
					//=-=|Process external css/js files|=-=//
					if (data.dataset.files!='') {
						var ext,gph,tail,files=data.dataset.files.split(',');
						dbg(files.length + ' Required files - Loading...');/*|DEBUG|*/
						for (var i=0;i<files.length;i++){
							ext=M$['gex'](files[i]);
							path=M$.asset(data.dataset.publisher,data.dataset.folder+M$.getDB(11)+ext,files[i]);
							if(ext=='js'){
					...
					processCode('style',data.getElementsByTagName('style'));
					processCode('script',data.getElementsByTagName('script'));
					
					dbg('Finalizing '+M$['VIEWPORT']+'...');
					//=-=|clear existing?|=-=//
					if (data.dataset.render==='replace') {
						dbg('Clearing '+M$['VIEWPORT']+'...');
						M$['gei'](M$['VIEWPORT']).innerHTML='';
						dbg('Done.');
					}
						
					//=-=|attach to DOM|=-=//	
					dbg('Attaching to DOM...');
				...
				else {dbg('Invalid tag (not INS)');	}
			} 
			else {dbg('Invalid data or no TagName');	}
		 ...
		 *
		 * @todo Move into intermediate file?
		 * @private
		 * @memberof Maestro
		 */
/*		 
		insertAds:function(target) {
			function createAdSlot(target) {
				var el=D$.createElement('ins');
				el.dataset.setAttribute('ad-client','ca-pub-7431399643348196');
				el.dataset.setAttribute('ad-slot','9001649268');
				el.dataset.setAtttribute('ad-format','auto');
				el.style.width=parent.availWidth;
				el.style.height=parent.availHeight;
...
*/
		...
		autoHeight:function() {
			...
			rows=rows.concat(B$.getElementsByClassName(this.getDB(9)));
			rows=rows.concat(B$.getElementsByClassName(this.getDB(10)));
		...
		getDB:function (idx,replace) {
			...
			HTML[5]='glyphicon glyphicon-remove';
			HTML[6]='Cancel fullscreen';
			HTML[7]='glyphicon glyphicon-fullscreen';
			HTML[8]='Enter Fullscreen';
			HTML[9]='row autoheight';
			HTML[10]='row-fluid autoheight';
			HTML[11]='/';
			HTML[12]='undefined';
			HTML[13]='application/javascript';
			HTML[23] = 'common';
		...
		toggleFullScreen:function (el) {
			...
			if ((D$.fullScreenElement && D$.fullScreenElement !== null) || (!D$.mozFullScreen && !D$.webkitIsFullScreen)) {
				...
				if (el.attributes[2]) {
					el.attributes[2].value=this.getDB(5);
					el.attributes[6].value=this.getDB(6);
				}
		  } else { 			
				...
				if (el.attributes[2]) {
					el.attributes[2].value=this.getDB(7);
					el.attributes[6].value=this.getDB(8);
				}
		...
		asset:function(publisher,repository,resource){
			...
			if (arguments.length==1) {
				var parts=publisher.split(M$.getDB(11));
				while (parts.length>1) {
					slice=parts.shift();
					path+=(slice!="")?slice+M$.getDB(11):M$.getDB(11);
				}
				path=this.PATH.stc +M$.getDB(11)+ path + parts[0];			
			}
			//get full path to folder but not resource name
			else if (arguments.length==2) {
				path=this.PATH.stc +M$.getDB(11)+ publisher + M$.getDB(11) + repository + M$.getDB(11);
			}
			/**|if three parameters encode simple path|**/
			else if (arguments.length==3) {
				//'/maestro/shared/applet/aceide'			
				path=([this.PATH.stc,publisher,repository,resource].join(M$.getDB(11)));
			} else 
				path='';
		 ...
		 * @expose
		 */
		vld : function(a){return!(null===a||M$.getDB(12)===typeof a||!1===a||""===a)},
		 ...
		 */
		gex : function(a){
			if (typeof a===M$.getDB(12)||a=='') return ''; else return /(?:\.([^.]+))?$/.exec(a)[1].toLowerCase()
		},
		 ...
		 */
		gph : function(a){
			if (typeof a===M$.getDB(12)||a=='') return ''; else a.replace(/^.*[\\\/]/, '');
		},
		...
		lds : function ($filetype, $id, $url, $tail, $after,$onLoad, $onError) {
			...
			a.onerror = ($onError)? $onError : function(){dbg('Error','Loading '+$url)};					/**DEBUG**/
		...
		xhr : function ($url, $onLoad, $onError, $method, $data, $trace) {
			...
			createCORSRequest = function (method, url) {
				...
				} else if (typeof XDomainRequest != M$.getDB(12)) {
				...
				// CORS not supported.
					xhr = null;
					dbg('error',maestro2['gfn']($url)+': CORS not supported.')
				}
				return xhr;
			}
			...
			makeCorsRequest = function ($url, $onLoad, $data) {
				...
				xhr.onload = function() {
					...
					else dbg('ERROR: [XHR] Ajax data received but no default handler found.');			/**DEBUG**/
					
				};

				xhr.onerror = function() {
					dbg('error '+this.responseText);
				};
	...
	try {
		dbg('Maestro Bootstrap...');		
		window['M$']=window['Maestro']=Maestro;
		M$.start();
	} catch (e) {
		dbg('Failed with error '+e);		
	}
})();
