...
extend(
	...
	/** @lends I$Pooled.prototype */
	{
		...
		setState:function(state){
			if (state<0||state>4) {
				this['warn']('Invalid State [QUEUED:0, LOADING:1, READY:2, FAILED:3]');//debug//
			} else {
				if (this.state!=state) {
					this['debug']('State changed '+this['state']+' to '+state+'.');
					this.state=state;
					switch (this.state) {
		...
		makeURI:function($data) {
			var _={path:null,fileName:null,ext:null,pub:''};
			
			try {
				this['debug']('Processing applet '+$data['dataset']['applet']+'...');
				if (M$['app']['config']['app']['publisher']) _.pub=M$['app']['config']['app']['publisher'];
				...
				this['header']['position']=(this['vld']($data['dataset']['position']))?$data['dataset']['position']:'auto';
				this['header']['vweight']=(this['vld']($data['dataset']['vweight']))?$data['dataset']['vweight']:'50';
				this['header']['hweight']=(this['vld']($data['dataset']['hweight']))?$data['dataset']['hweight']:'50';

				//=-=|if no publisher force local|=-=//
				if (this['header']['publisher']=='') {
					if (_.pub.length>0) {
						this['warn']('Cannot find pubilsher for applet. Using '+_.pub+' from CloudApp Config.');
						this['header']['publisher']=_.pub;
					} else {
						this['warn']('Cannot find pubilsher for applet. Will have to use shared folder.');
						this['header']['publisher']='shared';
						this['header']['local']=false;
					}
				}
				...
				//Prepare the URI
				if (!this['vld'](_.path=M$['app']['config']['getPath']('applets','publisher',this['header']['local']))){
					this['warn']('Publisher path not found.');
					return false
				}
				this.uri=_.path+_.fileName;
				// } else {
					// if (!this['vld'](_.path=M$['app']['getPath']('publisher',this['header'].publisher))){
						// this['warn']('Publisher path not found.');
						// return false
					// }
		...
		embed:function () {
			...
			this['element'] = $("div[data-applet='"+this['header'].name+"']").append(this['header'].html);
			this['element']['id']=this.id;
			
			//M$.bnd(this,this._onInterval),
			this['element']['onmouseenter']=M$['bnd'](this,function(){
				this['stateSaves']['border']=this['style']['border'];
				this['style']['border']='border: 3px solid #008000;'
			});
			
			tmp[0]['onmouseexit']=M$['bnd'](this,function(){
				this['style']['border']=this['stateSaves']['border'];
			});
			//if (M$['isDevMode']()) 
			//this.setDraggable();
			this.setState(3);
		...
		fetch:function(){
			...
			try {
				...
				_.applet = M$['app']['local']['getAppFile'](this['header'].name);
		...
		_onLoad:function($data) {
			...
			if ($data.tagName=='DIV' && $data['dataset']['name']) {
				...
				(function(a,d){
					...
					//var c = a.unpack(b);
					a.setState(2);
					M$['app']['applets'].scan($data.innerHTML,false);
					
				})(M$['app']['applets'].objects[$data['dataset']['name']],$data);			
				
			}
			
			//var level1 = this['parent']['applets'].getApplets($data.innerHTML,false);
			//for (var i=0;i<level1.length;i++){
			//	this.create(level1[i]);
			//}
			
			//this['parent']['applets'].scanAnchors($data.innerHTML);
...
extend(
	...
	/** @lends I$Factory.prototype */
	{
		...
		scan:function($html) {
			...
			//this.requestApplet({name:this['parent']['config']['app'].layout});		
		},
		...
		onHREFClick:function($evt) {
			try {
				this['parent']['applets'].setpage(this.name);
			} catch (e) {
				ast(null,e.stack);
			}
		},
		...
		setpage:function($applet_name) {
			...
			try {
				var $applet = this['parent']['applets'].use({name:$applet_name}), $tmp;
				if (!$applet) return;
					var $DIVs = this.getApplets("self");
					
					for (i=0;i<$DIVs.length;i++) {
						var $name=$($DIVs[i]).attr('data-applet');
						$tmp = this['parent']['applets'].use({name:$name});
						var tmp = $("div[data-applet='"+this['header'].name+"']").append($html);		
					}
							
				if (this['parent']['config']['options'].crossfade) {
...
/**
 * Layout Manager - Layouts are basically built-in special applets that just provide structure
 * and define areas that accept or reject applets with a certain role. Layout manager also handles the 
 * top to bottom left to right weight system.
 *
 * @class I$LayoutManager
 * @extends I$Interface
 * @expose 
 */
extend(
	...
	{		
		...
		/**
		 * These are the 5 different roles of an applet plus the layout option.
		 */
		Roles:[			
			'header',		
			'nav',
		...
		Zone:{
			name:'',
			role:'',				//+ or - for each of the roles in order, example to accept anything but header or footer -++++-
			Weight:{
				v:0,			//Vertical - higher the number the lower this area is on page 0-100
				h:0				//Horizonal - higher the number the farther right this area is on page 0-100
			}
		},
		...
		ZONES: [
			...
			['MiddleRight','++--+-',50,100],
			['MiddleBottom','++--+-',55,50],
			['FeaturetteBottom','-+-++-',60,50],
			['CarouselBottom','---++-',65,50],
			['NavBottom','-+----',85,50],
			['FloorLeft','++--+-',100,0],
			['Floor','++--+-',100,50],
			['FloorRight','++--+-',100,100]
		 ...
		 * @returns {string}
		 */
		toString:function (){return 'I$LayoutManager Class';}
	},
	/** @lends I$Interface.prototype */
	{
		/**
...
extend(
	...
	/** @lends I$Interface.prototype */
	{
		...
		getPath:function($what,$where,$appFolder) {
			...
			//var $base=window.location.protocol+P$['db'][2][0];
			var $base=window.location.protocol+'static.i2tmlabs.com/';
			if (window.location.href.indexOf('localhost:8080')!=-1) $base=location.origin+'/static';			
			...
			switch ($where.toLowerCase()) {
				...
				case 	'app': 		if ($appFolder!=true) 
										return this['Class']['path']['app']; 
									else
										$base+='/content/'+textize(this['config']['app']['publisher'])+'/assets/';
									break;	
			...
			//Publisher?
			if (($where=='publisher'&&$appFolder==true)||($where=='app'&&$appFolder==true)&&$what) $base+=textize(this['config']['app']['name'])+'/';
		...
		parseConfigFiles:function(){			
			var $whereVals=['app','publisher','shared','vendor'],
				$whatVals=['styles','scripts','applets','libraries'],
				$w,
				$files=[],
				self=this,
			...
			try {
				...
				for ($what in $res) {
					if ($whatVals.indexOf($what)!=-1) {
						for ($where in $res[$what]) {
							if ($whereVals.indexOf($where)!=-1) {
								tmp=$res[$what][$where].split(",");
								if (typeof tmp==="object"&&tmp.length>0) {
									for (i=0;i<tmp.length;i++)
										if (tmp[i] && tmp[i]!='')
											self['filesNeeded'].push(self['getPath']($what,$where,($where=='app'))+tmp[i]);								
								}	
							}
						}					
					}
		...
		_onConfigSuccess:function($req){
			...
			try {
				...
				this['error'](e,{},Array.prototype.slice.call(arguments, 0));
				return false;
			}
...
extend(
	...
	{		
		...
		Module : {
			...
			/**
			  ...
			  * @expose 
			  */
			built: 	"Fri, Apr 11, 2014  9:20:20 PM",
			/**
	...
	/** @lends I$Component.prototype */
	{
		...
		init:function ()
		{
			...
			this['applets'] = create('I$AppletFactory',this);					
		 ...
		 */
		setup:function(){
			P$['timer']['reset']();P$['timer']['start']();//DEBUG
			...
			this['gei']('base').href=this['config']['getPath']('assets','publisher',false);
			this['debug']('Document Base path set to Publishers Assets area.');
		},
		/**
		 * This is triggered by the core after this interface is completely
		 * initialized.
		...
		onReady:function(){
			this['_super']();	
			this['debug'](this.toString()+' Initialized.');
			this['createMeta']();
			this['getlayout']();
		...
		initEvents:function() {
			$(window).resize(function(){
				...
				this.resizeTimer = setTimeout(this._onResizeComplete, 250);
			});		
		},
		...
		checkScreenMode:function() {		// No L/R		 L only     L & R      ALL 
			var current = this.mode;		//0 = <768, 1 = 769-991, 2= 960-1920, 3=1921 >
			/*
			if (window.innerWidth < 768 && current != 0) {
				// We changed down to mode 0  = No Sidebars + Mobile menu
				this.mode = 0;
			} else if ((window.innerWidth > 768 && window.innerWidth < 992) && current != 1) {				
				// We changed into mode 1  = No Right Sidebar + Mobile menu
				this.mode = 1;
			} else if ((window.innerWidth > 992 && window.innerWidth < 1200) && current != 2) {				
				// We changed into mode 2	= Both sidebars no mobile menu
				this.mode = 2;
			} else if (window.innerWidth > 1200 && window.innerWidth < 1600) && current != 3) {
				// We changed into mode 3   = Both sidebars no mobile menu
				this.mode = 3;
			} else if (window.innerWidth > 1600 && window.innerWidth != 1920) && current != 4) {
				// We changed into mode 4   = Both sidebars no mobile menu
				this.mode = 4;
			} else if (window.innerWidth == 1920 && window.innerHeight == 1080) && current != 5) {
				// We changed into mode 5   = Both sidebars no mobile menu - FULL HD
				this.mode = 5;
			}
		...
		addMeta:function($name,$content) {
			try {
				...
				M$['error'](this,e,_,Array.prototype.slice.call(arguments, 0));
			}
		},
		...
		onLayoutLoaded:function(html) {
			this['debug']('Layout loaded, processing');
			this['applets']['scan'](html);		
			var wrapper=this['gei']('wrapper').innerHTML=html;
			this['debug']('Layout processing complete.');
		},
		...
		_onInterval:function($steps,$count,$fps){
			if (this['scripts']&&this['scripts']['length']>0) {
				...
				if (!this['finished']) this['debug']('Queue empty, idle...');
				this['finished']=true;
			...
			if (this['finished']&&this['ready'] && !this['readyCalled']) {
				if (typeof(window.nanoREADY) === "function") {
					M$['debug']('nanoREADY found, Invoking...');
					window.nanoREADY();
				}
				...
				M$['pld']();				
				P$['timer']['stop']();
				this['debug']('Total application time '+P$['timer']['elapsed']()+'ms');//DEBUG
			} 
		},
