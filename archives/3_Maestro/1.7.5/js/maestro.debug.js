	...
	*/
	var Maestro = {
		...
		click:function(url,publisher) {
			...
			file=M$.PATH.stc + path + 'applet.' + parts[0] + '.html';
			
			
			if (ga) 
				ga('send', 'event', 'button', 'click', url);
				
			var applet;   /**   // same as /appname/  /shared/ shared publisher, /abcpub/assets/js/  **/
			if(typeof publisher==='string'){
				//remote applet  /static loc/publisher/assets/shared||app/asset/
				//               /static.i2tmlabs.com/i2tm/shared/gizmo/js||css||applet|\etc 
				//applet=M$.PATH.stc+publisher+'assets/'++"/applets/"+url+'.html';
	//			applet=M$.assetPath(publisher,el.dataset.folder,'applet');
			} else {
				//local applet
//				applet=M$['APP_BASE']+"applets/"+url+'.html';
			}
				applet=M$.assetPath(publisher,el.dataset.folder,'applet');
		 ...
		 * @expose
		 */
		assetPath:function(publisher,entity,asset){
			//return this.PATH.stc+publisher+'/assets/'+entity+'/'+asset+'/';
			return this.PATH.stc+publisher+'/'+entity+'/'+asset+'/';
		},
		...
		_onLoad:function(data) {
			var done=M$.parseApplet(data);	//is applet?
											//is what?
											//is huh?
		},
			...
			if (data&&data['tagName']){
				if (data['tagName']==='M-NANO' && data.dataset.component && data.dataset.component==='applet') {
					...
					if (data.dataset.files!='') {
						...
						for (var i=0;i<files.length;i++){
							ext=M$['gex'](files[i]);
							path=M$.assetPath(data.dataset.publisher,data.dataset.folder,ext);
							if(ext=='js'){
								
								M$.lds('script',M$['gfn'](files[i]),path+files[i],true);
							} else if (ext=='css'){
								M$.lds('link',M$['gfn'](files[i]),path+files[i],false);
							}
						}
					}
