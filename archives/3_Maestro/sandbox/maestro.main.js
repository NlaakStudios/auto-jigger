...
(function(cfg) {
	...
	/**
	 * Console dbg - removed in release version
	 *
	 * @expose
	 * @param {string} msg
	...
	*/
	var Maestro=(function(){
		...
		que=function(files){
			...
			dbg(M$._I.scripts.length + 'Files in que.');
		},
		...
		start=function(self) {
			...
			if(location.protocol=='file:') {
				dbg('You cannot run document locally. Please access through web server.','error');
				return false
			}
		...
		parseApplet=function(data,$url) {
			...
			if (!this.mNanoOk) {
				data=replaceAll(data,'m-nano','ins');
			}
			...
			if (data&&data['tagName']){
				if (data['tagName']==='M-NANO' && data.dataset.component && data.dataset.component==='applet') {
					...
					dbg('Yes, Applet ' + data.id + ' by ' + data.dataset.publisher + ' processing.');/*|DEBUG|*/
					...
					dbg('Processing embed styling...');/*|DEBUG|*/
					var code,src='';
					...
					if (code.length>0) {
						...
						a.id=data.id+"_css";
						a.innerText=src;
						dbg('Attaching style ' +a.id+ ' to Head.');/*|DEBUG|*/
						H$.appendChild(a);
					}
					...
					dbg('Applet processing complete.');/*|DEBUG|*/
					return true;
				}
			}
		...
		lput=function(key,value){
			if (typeof localStore==='undefined') return; 
			try {
				localStorage.setItem(key.toLowerCase(), JSON.stringify(value));
			} catch (e) {
				dbg('ERROR writing '+key+' to localstorage');
			}
		},
		

		// Retrieve the object from storage
		lget=function(key,value){
			if (typeof localStore==='undefined') return; 
			try {
				return localStorage.getItem(key.toLowerCase())||value;
			} catch (e) {
				dbg('ERROR reading '+key+' from localstorage');
			}
		},
		...
		xhr=function ($url, $onLoad, $onError, $method, $data, $trace) {
			...
			createCORSRequest = function (method, url) {
				...
				// CORS not supported.
					xhr = null;
					dbg(M$['gfn']($url)+': CORS not supported.')
				}
				return xhr;
			}
			...
			makeCorsRequest = function ($url, $onLoad, $data) {
				...
				xhr.onerror = function() {
					dbg(this.responseText,'ERROR');
				};

				if (xhr&&xhr.send) xhr.send();
				 
				if (M$['gei']('LED-Rec')) M$['gei']('LED-Rec').className='led round green-off';
