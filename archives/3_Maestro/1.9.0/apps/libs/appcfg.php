...
class MCloudAppCfg
{
	...
	private function initialize($data) {
		if (is_array($data)) {
			...
			//Verify APP Section
			$this->validate('name',$data['app'],'store');									//required, lowercase
			$this->validate('version',$data['app'],'1.0.0');								//required
			$this->validate('uuid',$data['app'],'91e714bd-9f30-4b89-8ce6-7a411c9e9bca');	//required
			$this->validate('role',$data['app'],'application');								//required
			$this->validate('publisher',$data['app'],'maestro');							//required
	...
	private function getApplication() {
		...
		if (!isset($this->cfg['app']['theme']) || !array_key_exists($this->cfg['app']['theme'], MaestroColor::$THEMES)) {
	...
	protected function load($compiled=true) {
		...
		//if (!$compiled) {
			...
			if(file_exists($file)) {
				...
				$this->cfgerr=true;
				$this->owner->log('ERROR: cannot load application configuration file.\n' . $file);
				//Error loading config? set defaults and load app store with error
				$this->json = [];
			}
		//} else {
		...
		$this->cfg=$this->initialize($this->json_clean_decode($this->json,true));
		return true;
	}
